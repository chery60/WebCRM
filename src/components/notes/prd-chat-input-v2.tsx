...
const AI_MODELS: { value: AIProviderType; label: string; description?: string }[] = [
  { value: 'openai', label: 'GPT-4o', description: 'Best for complex reasoning' },
  { value: 'anthropic', label: 'Claude Sonnet 4', description: 'Best for writing' },
  { value: 'gemini', label: 'Gemini 2.0 Flash', description: 'Fast and capable' },
];
...
export function PRDChatInputV2({
  value,
  onChange,
  onSubmit,
  onStop,
    ...
    <div className="border-t border-border bg-background p-4 space-y-3 sticky bottom-0 z-10">
        ...
        <div className="flex-1 relative">
          ...
          {isStreaming ? (
            <Button
              size="icon"
              variant="ghost"
              className="absolute right-2 bottom-2 h-8 w-8 text-destructive hover:text-destructive"
              onClick={onStop}
            >
            ...
            <Button
              ...
              className={cn(
                ...
                value.trim() ? "text-primary hover:text-primary" : "text-muted-foreground"
              )}
              onClick={handleSend}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Select
          ...
          onValueChange={(val) => onProviderChange(val as AIProviderType)}
